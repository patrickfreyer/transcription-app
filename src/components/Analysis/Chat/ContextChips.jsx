...
const ContextChips = () => {
  ...
  const actualContextCount = searchAllTranscripts
    ? transcripts.length
    ...
    <div className="flex items-center gap-3 flex-wrap">
      ...
      <div className="flex items-center gap-2">
        <span className="text-xs font-semibold text-foreground">
          ...
          {searchAllTranscripts && (
            <span className="ml-1.5 px-1.5 py-0.5 bg-primary text-white text-[10px] rounded uppercase font-bold">
              All
            </span>
          )}
        </span>
        <div className="group relative">
          ...
          <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-2 bg-surface-secondary text-foreground text-xs rounded-lg shadow-lg opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity whitespace-nowrap z-50 border border-border max-w-xs">
            {searchAllTranscripts
              ? 'AI searches across all transcripts using vector search'
      ...
      {/* Show RAG mode indicator if enabled */}
      {searchAllTranscripts ? (
        <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-primary bg-opacity-15 text-primary text-xs font-semibold rounded-full border border-primary">
          <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
          </svg>
          <span>All Transcripts (Vector Search)</span>
          <button
            onClick={() => {
        ...
        /* Show summary chip when >10 selected but not in RAG mode */
      ...
      {/* Show current transcript if no context selected and NOT in RAG mode */}
      {!searchAllTranscripts && selectedContextIds.length === 0 && selectedTranscript && (
