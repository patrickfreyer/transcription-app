    ...
    <div className="w-full max-w-xs mx-auto">
      ...
      <div className="relative">
        <select
          ...
          disabled={disabled}
          className="w-full px-4 py-2.5 pr-10 bg-surface border border-border rounded-xl text-sm font-medium text-foreground appearance-none cursor-pointer focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition-all disabled:opacity-50 disabled:cursor-not-allowed hover:border-strong dark:bg-surface-secondary"
        >
          {sources.map((source) => (
        ...
        <div className="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
          <svg className="w-4 h-4 text-foreground-secondary" viewBox="0 0 20 20" fill="currentColor">
